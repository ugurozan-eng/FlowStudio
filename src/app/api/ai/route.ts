  ...
  try {
    ...
    if (action === 'revise') {
      ...
      switch (stage) {
        ...
        case 'ElevenLabs':
          basePrompt = language === 'en'
            ? `Convert the following video script into a natural, spoken-word format for ElevenLabs Text-to-Speech.\n\nDO NOT SHORTEN OR SUMMARIZE. Keep ALL content.\n\nCRITICAL RULES — ElevenLabs reads EVERY character aloud, so:\n❌ NEVER use brackets like [pause], [tense], [excited], [SCENE CHANGE] — ElevenLabs will read them as words\n❌ NEVER use symbols like ↑ or ↓ — ElevenLabs will say "up arrow"\n❌ NEVER use SSML tags like <break> or <emphasis> — keep it pure natural text\n\n✅ INSTEAD use ONLY natural text techniques:\n- "..." (ellipsis) for natural pauses — ElevenLabs pauses on ellipsis\n- ALL CAPS for words that need vocal emphasis — ElevenLabs naturally emphasizes shouted words\n- A blank line (double enter) between paragraphs for longer pauses between scenes\n- Short sentences for fast sections, longer sentences for slower, calmer sections\n- Natural conversational language that a real human would speak\n\n--- SEPARATOR (scene changes) ---\nUse this exact visual separator on its own line between major scene shifts. ElevenLabs will skip it but it marks scene boundaries for later storyboard creation.\n\nSCRIPT:\n${currentText}`
            : `Aşağıdaki video scriptini ElevenLabs Text-to-Speech için doğal konuşma formatına dönüştür.\n\nKISALTMA, ÖZETLEME. Tüm içeriği koru.\n\nKRİTİK KURALLAR — ElevenLabs HER KARAKTERİ sesli okur, bu yüzden:\n❌ ASLA köşeli parantez KULLANMA: [pause], [tense], [heyecanlı], [SAHNE DEGISIMI] gibi — ElevenLabs bunları kelime olarak okur\n❌ ASLA ↑ veya ↓ sembolleri KULLANMA — ElevenLabs "yukarı ok" diye okur\n❌ ASLA SSML etiketleri KULLANMA — saf doğal metin kullan\n\n✅ BUNUN YERİNE sadece doğal metin teknikleri:\n- "..." (üç nokta) doğal duraksamalar için — ElevenLabs üç noktada duraksar\n- BÜYÜK HARF vurgulanacak kelimeler için — ElevenLabs doğal olarak vurgular\n- Paragraflar arası boş satır (çift enter) sahneler arası daha uzun duraksamalar için\n- Hızlı bölümler için kısa cümleler, sakin bölümler için daha uzun cümleler\n- Gerçek bir insanın konuşacağı doğal, sohbet eder gibi bir dil\n\n--- AYIRICI (sahne değişimleri) ---\nBüyük sahne geçişlerinde bu görsel ayırıcıyı kendi satırında kullan. ElevenLabs okumaz ama sonraki storyboard için sahne sınırlarını belirler.\n\nSENARYO:\n${currentText}`;
          break;
        case 'Storyboard': {
